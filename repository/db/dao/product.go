...
func (dao *ProductDao) GetProductById(id uint) (product *model2.Product, err error) {
	err = dao.DB.Model(&model2.Product{}).Where("id = ?", id).First(&product).Error
	return
}
...
func (dao *ProductDao) UpdateProduct(pId uint, product *model2.Product) error {
	return dao.DB.Model(&model2.Product{}).Where("id = ?", pId).Updates(&product).Error
}

// SearchProduct 搜索商品
func (dao *ProductDao) SearchProduct(info string, page model2.BasePage) (products []*model2.Product, err error) {
	err = dao.DB.Model(&model2.Product{}).Where("name LIKE ? OR info LIKE ?", "%"+info+"%", "%"+info+"%").Offset((page.PageNum - 1) * page.PageSize).Limit(page.PageSize).Find(&products).Error
	return
}
